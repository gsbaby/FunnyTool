...
def message_askyesno(infoText):
    top = tkinter.Tk()  # *********
    top.withdraw()  # ****实现主窗口隐藏
    top.update()  # *********需要update一下
    if ('Black' in infoText):
        txt = tkinter.messagebox.askokcancel('提示', '游戏结束，黑子胜利！再来一局！')
    else:
        txt = tkinter.messagebox.askokcancel('提示', '游戏结束，白子胜利！再来一局！')
    top.destroy()
    return txt
            ...
            if event.type == QUIT:
                ...
                if 25 <= x <= 725 and 25 <= y <= 725 and ((x - 25) % 50 <= level or (x - 25) % 50 >= 0) and (
                    ...
                    if Judge(m, n, color, ch.a, 4):
                        ...
                        a = message_askyesno('GAME OVER,Black is win!')
                        print(a)
                        if (a):
                            now = chess()
                            satrtGUI(now)
                    ...
                    if Judge(x, y, color, ch.a, 4):
                        ...
                        a = message_askyesno('GAME OVER,White is win!')
                        if (a):
                            now = chess()
                            satrtGUI(now)
                        break
