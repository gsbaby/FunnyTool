...
def Button():
    ...
    a = 'https://www.administrator5.com/admin.php?url=' if varRadio.get() else 'http://www.a305.org/flv.php?url='
...
if __name__ == '__main__':
    ...
    helpmenu.add_cascade(label='帮助(H)', command=helpmenu)
    ...
    lab_movie_link = tkinter.Label(root, text='视频播放连接')
    ...
    button_move_link = tkinter.Button(root, text='清空', command=qk)
    button_move_link.place(x=440, y=60, width=30, height=20)

    lab_remind = tkinter.Label(root, text='将视频连接复制到框内，点击播放VIP视频')
    ...
    button_movie1 = tkinter.Button(root, text='腾讯视频', command=opentx)
    button_movie1.place(x=180, y=200, width=100, height=60)

    button_movie1 = tkinter.Button(root, text='优酷视频', command=openyq)
    button_movie1.place(x=300, y=200, width=100, height=60)
